...
exports.revokeApiKey = async (req, res) => {
    try {
        ...
        res.json({ success: true, message: 'API key revoked.', data: serializeApiKey(apiKey) });
    } catch (err) {
...
exports.suspendAdmin = async (req, res) => {
    try {
        ...
        if (found.source === 'user') {
            ...
            found.doc.suspensionReason = reason || 'Suspended by super admin';
        }
...
exports.unsuspendAdmin = async (req, res) => {
    try {
        ...
        console.error('unsuspendAdmin error:', err);
        res.status(500).json({ success: false, error: err.message });
    }
};

exports.activateAdmin = exports.unsuspendAdmin;

exports.listAvailablePermissions = async (req, res) => {
    res.json({ success: true, data: ROLES_WITH_PERMISSION });
};
